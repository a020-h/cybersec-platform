...
export default function LandingHero() {
  ...
  // ✅ نص ثابت أولاً — لا typing حتى بعد LCP
  ...
  useEffect(() => {
    ...
    if ('requestIdleCallback' in window) {
      requestIdleCallback(fetchStats, { timeout: 3000 })
    } else {
      setTimeout(fetchStats, 1500)
    }
  }, [])

  // ✅ Typing effect — يبدأ فقط بعد LCP (2.5 ثانية)
  useEffect(() => {
    const startTyping = () => {
      ...
      tid = setTimeout(type, 300)
      return () => clearTimeout(tid)
    }

    // ✅ ابدأ بعد 2.5s — بعد ما LCP يتحمل
    const t = setTimeout(startTyping, 2500)
        ...
        <section style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', padding: '110px 48px 80px', maxWidth: '1280px', margin: '0 auto' }}>
          <div className="hero-grid" style={{ display: 'flex', alignItems: 'center', gap: '60px', width: '100%' }}>
            <div style={{ flex: 1 }}>
              ...
              {/* ✅ LCP element — no animation delay, renders in SSR */}
              <h1 className="hero-title" style={{ fontSize: '54px', fontWeight: 900, lineHeight: 1.2, marginBottom: '18px', color: 'white' }}>
                تعلّم<br /><span style={{ color: '#00cc70' }}>الأمن السيبراني</span><br />بالعربي
              </h1>

              {/* ✅ نص ثابت حتى يبدأ الـ typing */}
              <div style={{ fontSize: '17px', color: '#5a7a90', marginBottom: '10px', fontFamily: "var(--font-space-mono),monospace", height: '26px', overflow: 'hidden' }}>
                ...
                <span style={{ color: '#a0c0d8' }}>
                  {typingStarted ? matrixText : 'اختبار الاختراق...'}
                </span>
