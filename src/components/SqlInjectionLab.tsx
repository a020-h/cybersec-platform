...
const CHALLENGES: Challenge[] = [
  { id: 1, title: 'الدخول بدون كلمة مرور', description: 'الموقع المستهدف يحتوي على نموذج تسجيل دخول. حاول الدخول كـ admin بدون معرفة كلمة المرور.', hint: "جرب: admin' -- في حقل اسم المستخدم", solution: "admin' --", points: 50 },
  { id: 2, title: 'استخراج قاعدة البيانات', description: 'استخرج اسم قاعدة البيانات الحالية باستخدام UNION SELECT.', hint: "جرب: ' UNION SELECT database(),2,3 -- في حقل البحث", solution: "' UNION SELECT database(),2,3 --", points: 75 },
  { id: 3, title: 'سرقة بيانات المستخدمين', description: 'استخرج جدول المستخدمين من قاعدة البيانات.', hint: "جرب: ' UNION SELECT username,password,3 FROM users --", solution: "' UNION SELECT username,password,3 FROM users --", points: 100 },
  { id: 4, title: 'تجاوز الصلاحيات', description: 'ادخل كمستخدم admin باستخدام OR injection.', hint: "جرب: ' OR '1'='1 في حقل كلمة المرور", solution: "' OR '1'='1", points: 125 }
]
...
function processSQL(input: string, field: 'username' | 'password' | 'search'): { success: boolean; data: any; query: string } {
  ...
  if (field === 'username') {
    ...
    if (inp.includes("'") && (inp.includes('--') || inp.includes('#'))) {
      ...
      if (user || inp.includes('admin')) return { success: true, data: { user: user || fakeDB.users[0] }, query }
    }
    return { success: false, data: null, query }
  }
  if (field === 'password') {
    const query = `SELECT * FROM users WHERE username='admin' AND password='${input}'`
    if (inp.includes("'") && inp.includes('or') && inp.includes('1')) return { success: true, data: { user: fakeDB.users[0] }, query }
    return { success: false, data: null, query }
  }
  if (field === 'search') {
    const query = `SELECT id,name,price FROM products WHERE name LIKE '%${input}%'`
    if (inp.includes('union') && inp.includes('select')) {
      if (inp.includes('database()')) return { success: true, data: { union: true, result: [[fakeDB.dbName, '2', '3']] }, query }
      if (inp.includes('from users')) return { success: true, data: { union: true, result: fakeDB.users.map(u => [u.username, u.password, u.role]) }, query }
    }
    return { success: true, data: { products: fakeDB.products.filter(p => p.name.toLowerCase().includes(inp)) }, query }
  }
  return { success: false, data: null, query: '' }
}

function BrowserSim({ solved, onSolve, currentChallenge }: { solved: Set<number>; onSolve: (id: ChallengeId, points: number) => void; currentChallenge: ChallengeId }) {
  ...
  const handleLogin = () => {
    ...
    if (r1.success) {
      setLoggedIn(true); setLoggedUser(r1.data.user)
      ...
      setTimeout(() => setPage('dashboard'), 800); return
    }
    const r2 = processSQL(password, 'password')
    if (r2.success) {
      setLoggedIn(true); setLoggedUser(r2.data.user)
  ...
  const url = page === 'home' ? 'http://vuln-shop.local/' : page === 'login' ? 'http://vuln-shop.local/login' : page === 'search' ? 'http://vuln-shop.local/search' : 'http://vuln-shop.local/dashboard'
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', fontFamily: "'Cairo', sans-serif" }}>
      <div style={{ background: '#1a1a2e', borderBottom: '1px solid #ff3366', padding: '8px 12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <div style={{ flex: 1, background: '#0d0d1a', border: '1px solid #ff336644', borderRadius: 4, padding: '3px 10px', fontSize: 11, color: '#ff6688', fontFamily: 'monospace' }}>🔓 {url}</div>
      </div>
      <div style={{ background: '#0d0d1a', borderBottom: '1px solid #1a1a3a', padding: '8px 16px', display: 'flex', gap: '12px' }}>
        ...
        {loggedIn && <span style={{ marginRight: 'auto', color: '#00ff88', fontSize: 12 }}>👤 {loggedUser?.username}</span>}
      </div>
        ...
        {page === 'login' && (
          <div style={{ maxWidth: 360, margin: '0 auto', paddingTop: 20 }}>
            ...
            <div style={{ background: '#0d0d1a', border: '1px solid #1a1a3a', borderRadius: 8, padding: 20 }}>
              ...
              <button onClick={handleLogin} style={{ width: '100%', background: '#ff3366', color: 'white', border: 'none', borderRadius: 4, padding: '10px', fontFamily: "'Cairo', sans-serif", fontWeight: 700, cursor: 'pointer', fontSize: 14 }}>دخول</button>
              {loginResult && (
            ...
            <div style={{ marginTop: 12, background: '#0d0d1a', border: '1px solid #1a1a3a', borderRadius: 6, padding: 12 }}>
              ...
              <code style={{ color: '#4488ff', fontSize: 10, display: 'block', marginTop: 4 }}>{`"SELECT * FROM users WHERE username='"+input+"' AND password='"+pw+"'"`}</code>
            </div>
          </div>
        )}
        {page === 'search' && (
          <div style={{ maxWidth: 500, margin: '0 auto', paddingTop: 20 }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
              ...
              <button onClick={handleSearch} style={{ background: '#ff3366', color: 'white', border: 'none', borderRadius: 4, padding: '8px 16px', cursor: 'pointer', fontFamily: "'Cairo', sans-serif", fontSize: 13 }}>بحث</button>
            </div>
            {searchResult && (
              <div>
                      ...
                      <tbody>
                        {searchResult.data.result.map((row: string[], i: number) => (
                          <tr key={i} style={{ borderBottom: '1px solid #0d2a0d' }}>
                            {row.map((cell, j) => <td key={j} style={{ padding: '6px 10px', color: j === 1 ? '#ff6688' : '#a0ffa0', fontFamily: 'monospace' }}>{cell}</td>)}
                          </tr>
                        ))}
                      </tbody>
                    </table>
...
function Terminal({ onCommand, lines }: { onCommand: (cmd: string) => void; lines: TerminalLine[] }) {
  ...
  const submit = () => {
    ...
    setHistory(h => [input, ...h]); setHistIdx(-1); onCommand(input); setInput('')
  }

  const colorMap: Record<TerminalLine['type'], string> = { input: '#00ff88', output: '#a0c8d8', error: '#ff4444', success: '#ffd700', system: '#666' }

  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#050508', fontFamily: 'monospace' }} onClick={() => inputRef.current?.focus()}>
...
export default function SqlInjectionLab({ onComplete }: { onComplete?: (score: number) => void }) {
  ...
  const addLines = (newLines: TerminalLine[]) => setTerminalLines(prev => [...prev, ...newLines])

  const handleSolve = (id: ChallengeId, points: number) => {
    if (solved.has(id)) return
    setSolved(prev => new Set([...prev, id]))
    setTotalScore(prev => prev + points)
    addLines([{ type: 'system', content: '' }, { type: 'success', content: `🎯 تحدي #${id} محلول! +${points} نقطة` }, { type: 'system', content: '' }])
    if (solved.size + 1 === 4) { setTimeout(() => setStage('complete'), 1500); onComplete?.(totalScore + points) }
  }
  ...
  const handleTerminalCommand = (cmd: string) => {
    ...
    if (c === 'help') {
      ...
      const ch = CHALLENGES.find(ch => ch.id === currentChallenge)!
      addLines([{ type: 'output', content: `التحدي ${ch.id}/4: ${ch.title}` }, { type: 'output', content: ch.description }, { type: 'output', content: `النقاط: ${ch.points}` }])
    } else if (c === 'hint') {
      const ch = CHALLENGES.find(ch => ch.id === currentChallenge)!
      addLines([{ type: 'success', content: `💡 ${ch.hint}` }])
    } else if (c === 'score') {
      addLines([{ type: 'output', content: `النقاط: ${totalScore}` }, { type: 'output', content: `التحديات المحلولة: ${solved.size}/4` }])
    } else if (c === 'next') {
      if (currentChallenge < 4) { setCurrentChallenge(prev => (prev + 1) as ChallengeId); addLines([{ type: 'system', content: `انتقلت للتحدي ${currentChallenge + 1}` }]) }
      else addLines([{ type: 'output', content: 'هذا هو التحدي الأخير' }])
    } else if (c === 'clear') {
      setTerminalLines([])
    } else if (c.startsWith('sqlmap')) {
      addLines([{ type: 'error', content: 'sqlmap غير متاح في هذا Lab 😄' }, { type: 'output', content: 'حاول يدوياً! هذا أفيد للتعلم' }])
    } else {
  ...
  if (stage === 'intro') {
    return (
      <div style={{ background: 'linear-gradient(135deg, #050508 0%, #0a0a1a 50%, #0d0515 100%)', minHeight: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Cairo', sans-serif", padding: 24, direction: 'rtl' }}>
        <div style={{ maxWidth: 600, width: '100%' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 32 }}>
            <div style={{ background: '#ff336622', border: '1px solid #ff3366', borderRadius: 8, padding: '6px 14px', fontSize: 12, color: '#ff6688' }}>🧪 Lab تفاعلي</div>
            <div style={{ background: '#ffd70022', border: '1px solid #ffd70066', borderRadius: 8, padding: '6px 14px', fontSize: 12, color: '#ffd700' }}>⚡ مستوى: مبتدئ-متوسط</div>
            <div style={{ background: '#00ff8822', border: '1px solid #00ff8866', borderRadius: 8, padding: '6px 14px', fontSize: 12, color: '#00ff88' }}>🏆 350 نقطة</div>
          </div>
          ...
          <div style={{ marginBottom: 32 }}>
            {CHALLENGES.map((ch, i) => (
              <div key={ch.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 16px', marginBottom: 8, background: '#0d0d1a', border: '1px solid #1a1a3a', borderRadius: 8, animation: `fadeIn 0.4s ease ${i * 0.1}s both` }}>
                <div style={{ width: 32, height: 32, borderRadius: '50%', background: '#ff336622', border: '1px solid #ff336666', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ff3366', fontFamily: 'monospace', fontSize: 13, fontWeight: 700, flexShrink: 0 }}>{ch.id}</div>
                <div style={{ flex: 1 }}>
          ...
          <button onClick={() => setStage('lab')} style={{ width: '100%', padding: '16px', background: 'linear-gradient(135deg, #ff3366, #cc1144)', color: 'white', border: 'none', borderRadius: 10, fontFamily: "'Cairo', sans-serif", fontSize: 18, fontWeight: 900, cursor: 'pointer', boxShadow: '0 0 30px #ff336644', transition: 'all 0.3s' }}>
            🚀 ابدأ Lab الآن
          </button>
  ...
  if (stage === 'complete') {
    return (
      <div style={{ background: '#050508', minHeight: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Cairo', sans-serif", direction: 'rtl', padding: 24 }}>
        <div style={{ textAlign: 'center', maxWidth: 480 }}>
          ...
          <button onClick={() => window.location.href = '/dashboard'} style={{ marginTop: 24, width: '100%', padding: '14px', background: 'linear-gradient(135deg, #ff3366, #cc1144)', color: 'white', border: 'none', borderRadius: 10, fontFamily: "'Cairo', sans-serif", fontSize: 16, fontWeight: 900, cursor: 'pointer', boxShadow: '0 0 20px #ff336644' }}>
            ← العودة للداشبورد
          </button>
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#050508', fontFamily: "'Cairo', sans-serif", direction: 'rtl' }}>
      {/* Top Bar */}
      <div style={{ background: '#0a0a18', borderBottom: '1px solid #1a1a3a', padding: '10px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {CHALLENGES.map(ch => (
            <button key={ch.id} onClick={() => setCurrentChallenge(ch.id)}
              style={{ width: 32, height: 32, borderRadius: '50%', border: `2px solid ${solved.has(ch.id) ? '#00ff88' : ch.id === currentChallenge ? '#ff3366' : '#1a1a3a'}`, background: solved.has(ch.id) ? '#00ff8822' : ch.id === currentChallenge ? '#ff336622' : 'transparent', color: solved.has(ch.id) ? '#00ff88' : ch.id === currentChallenge ? '#ff3366' : '#444', cursor: 'pointer', fontSize: 12, fontWeight: 700, fontFamily: 'monospace' }}>
      ...
      {/* Challenge Banner */}
      <div style={{ background: 'linear-gradient(135deg, #ff336611, #00000000)', borderBottom: '1px solid #ff336633', padding: '12px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => setShowHint(!showHint)} style={{ background: '#ffd70022', border: '1px solid #ffd70066', color: '#ffd700', padding: '6px 12px', borderRadius: 6, cursor: 'pointer', fontSize: 12, fontFamily: "'Cairo', sans-serif" }}>💡 تلميح</button>
          {currentChallenge < 4 && (
            <button onClick={() => setCurrentChallenge(prev => (prev + 1) as ChallengeId)} style={{ background: '#1a1a3a', border: '1px solid #2a2a4a', color: '#888', padding: '6px 12px', borderRadius: 6, cursor: 'pointer', fontSize: 12, fontFamily: "'Cairo', sans-serif" }}>التالي →</button>
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid #1a1a3a', background: '#080814' }}>
        {(['browser', 'terminal', 'hints'] as TabType[]).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            style={{ padding: '10px 20px', border: 'none', borderBottom: `2px solid ${activeTab === tab ? '#ff3366' : 'transparent'}`, background: 'transparent', color: activeTab === tab ? '#ff3366' : '#555', cursor: 'pointer', fontSize: 13, fontFamily: "'Cairo', sans-serif" }}>
      ...
      <div style={{ flex: 1, overflow: 'hidden' }}>
        {activeTab === 'browser' && <BrowserSim solved={solved} onSolve={handleSolve} currentChallenge={currentChallenge} />}
        {activeTab === 'terminal' && <Terminal lines={terminalLines} onCommand={handleTerminalCommand} />}
        {activeTab === 'hints' && (
          <div style={{ padding: 24, overflow: 'auto', height: '100%', direction: 'rtl' }}>
            ...
            <div style={{ background: '#0d0d1a', border: '1px solid #1a1a3a', borderRadius: 8, padding: 20, marginBottom: 16 }}>
              <p style={{ color: '#a0c8d8', fontSize: 14, lineHeight: 1.8 }}>SQL Injection تحدث عندما يأخذ الكود مدخل المستخدم ويضعه مباشرة في SQL Query بدون تنظيف.</p>
              <code style={{ display: 'block', background: '#050508', padding: 12, borderRadius: 6, marginTop: 12, color: '#ffd700', fontSize: 12, direction: 'ltr' }}>{"SELECT * FROM users WHERE username='" + "admin' --" + "'"}</code>
              <p style={{ color: '#888', fontSize: 12, marginTop: 8 }}>الـ -- يعلّق بقية الكود مما يجعل التحقق من كلمة المرور يُهمل</p>
            </div>
            {CHALLENGES.map(ch => (
              <div key={ch.id} style={{ background: '#0d0d1a', border: `1px solid ${solved.has(ch.id) ? '#00ff8844' : '#1a1a3a'}`, borderRadius: 8, padding: 16, marginBottom: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                  <span style={{ color: solved.has(ch.id) ? '#00ff88' : 'white', fontSize: 14, fontWeight: 700 }}>{solved.has(ch.id) ? '✅' : '🔒'} {ch.title}</span>
                  <span style={{ color: '#ffd700', fontFamily: 'monospace', fontSize: 13 }}>+{ch.points}</span>
                </div>
