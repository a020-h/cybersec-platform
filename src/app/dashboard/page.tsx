...
const BadgeChecker = dynamic(() => import('@/components/BadgeChecker'), { ssr: false })
...
export default function Dashboard() {
  ...
  // إشعار بالرسائل الجديدة عند إغلاق الشات
  useEffect(() => {
    if (showChat) { setUnreadChat(0); return }
  ...
  const loadStats = async (userId: string) => {
    ...
    if (completions) {
      ...
      const done = courses.filter(c => (progress[c.id] || 0) >= c.lessons).length
    ...
    const { data: ub } = await supabase
      .from('user_badges')
      .select('badge_id, earned_at, badges(name, icon, color, description)')
      .eq('user_id', userId)
  ...
  const badgesDisplay = allBadges.map(badge => ({
    ...badge,
    unlocked: userBadges.some((ub: any) => ub.badge_id === badge.id),
  }))
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; list-style:none; }
        ...
        .stat-card { background:rgba(10,21,32,0.8); border:1px solid #1a3a50; border-radius:14px; padding:16px 20px; transition:all 0.3s; position:relative; overflow:hidden; cursor:default; }
        .stat-card:hover { border-color:rgba(0,255,136,0.25); transform:translateY(-2px); }
        .course-card { background:rgba(10,21,32,0.9); border-radius:18px; overflow:hidden; cursor:pointer; transition:all 0.4s cubic-bezier(0.4,0,0.2,1); border:1px solid #1a3a50; position:relative; }
        .course-card:hover { transform:translateY(-8px); }
        .nav-btn { padding:7px 16px; border-radius:100px; font-family:'Cairo',sans-serif; font-size:13px; cursor:pointer; font-weight:700; transition:all 0.25s; border:1px solid; display:flex; align-items:center; gap:6px; white-space:nowrap; }
        ...
        .tab-btn { padding:9px 22px; border-radius:9px; border:none; font-family:'Cairo',sans-serif; font-size:14px; font-weight:700; cursor:pointer; transition:all 0.3s; }
        .badge-card { border-radius:14px; padding:16px 10px; text-align:center; transition:all 0.3s; cursor:default; position:relative; overflow:hidden; }
        .badge-card.unlocked:hover { transform:translateY(-3px); }
        .level-card { border-radius:18px; padding:20px 24px; min-width:260px; position:relative; overflow:hidden; }
        ...
        /* Chat floating */
        .chat-fab { position:fixed; bottom:28px; left:28px; z-index:200; width:56px; height:56px; border-radius:50%; background:linear-gradient(135deg,#00ff88,#00d4ff); border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:24px; box-shadow:0 8px 32px rgba(0,255,136,0.35); transition:all 0.3s; }
        .chat-fab:hover { transform:scale(1.1); box-shadow:0 12px 40px rgba(0,255,136,0.5); }
        .chat-panel { position:fixed; bottom:100px; left:28px; width:380px; height:520px; z-index:199; animation:slideUp 0.3s cubic-bezier(0.4,0,0.2,1); }
        @media (max-width: 768px) {
          ...
          .chat-panel { left:12px; right:12px; width:auto; bottom:90px; height:460px; }
          .chat-fab { bottom:20px; left:20px; }
        }
        @media (min-width:769px) { .mobile-menu-btn { display:none !important; } .mobile-menu-dropdown { display:none !important; } }
        @media (min-width:640px) and (max-width:1024px) { .courses-grid { grid-template-columns:repeat(2,1fr) !important; } .navbar { padding:0 24px; } }
      `}</style>
      ...
      {user && (
        <BadgeChecker userId={user.id} points={points} lessonsCompleted={lessonsCompleted} completedCourses={completedCourses} />
      )}

      {/* ── FLOATING CHAT ── */}
      {showChat && user && (
        <div className="chat-panel">
          <CommunityChat userId={user.id} username={username} avatar={avatar} />
        </div>
      )}
      <button className="chat-fab" onClick={() => { setShowChat(!showChat); setUnreadChat(0) }} title="شات المجتمع">
        {showChat ? '✕' : '💬'}
      ...
      <div style={{ minHeight: '100vh', position: 'relative', zIndex: 1 }} dir="rtl">
        ...
        <nav className="navbar">
          <div className="nav-logo">
            ...
            <span style={{ color: '#00ff88', textShadow: '0 0 20px rgba(0,255,136,0.5)' }}>CYBER</span>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '20px', marginBottom: '28px' }} className="hero-top">
              <div className="fade-up">
                ...
                <p style={{ color: '#7090a8', fontSize: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  {userBadges.length > 0 && (
                    <span style={{ color: '#ffd700', fontSize: '13px', background: 'rgba(255,215,0,0.08)', border: '1px solid rgba(255,215,0,0.2)', padding: '2px 10px', borderRadius: '100px' }}>
                      🏅 {userBadges.length} إنجاز
                    </span>
                  )}
                </p>
              </div>
            ...
            <div className="tab-root">
              {(['courses', 'progress'] as const).map(tab => (
                <button key={tab} className="tab-btn" onClick={() => setActiveTab(tab)}
                  style={{ background: activeTab === tab ? 'linear-gradient(135deg, #00ff88, #00d4aa)' : 'transparent', color: activeTab === tab ? '#050a0f' : '#7090a8', boxShadow: activeTab === tab ? '0 4px 16px rgba(0,255,136,0.3)' : 'none' }}>
              ...
              {courses.map((course, i) => {
                ...
                return (
                  <div key={course.id} className="course-card fade-up"
                    style={{ animationDelay: `${i * 0.08}s`, border: `1px solid ${isHovered ? course.color + '66' : isComplete ? course.color + '33' : '#1a3a50'}`, boxShadow: isHovered ? `0 20px 50px ${course.color}20, 0 0 0 1px ${course.color}20` : 'none' }}
                    ...
                    <div style={{ background: course.bg, padding: '24px 20px', position: 'relative', overflow: 'hidden', minHeight: '130px' }}>
                      ...
                      <div style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', alignItems: 'flex-end' }}>
                          <span style={{ background: `${course.color}18`, border: `1px solid ${course.color}35`, color: course.color, padding: '3px 10px', borderRadius: '100px', fontSize: '10px', fontFamily: 'Space Mono,monospace', fontWeight: '700' }}>{course.level}</span>
                          {isComplete && <span style={{ background: 'rgba(0,255,136,0.12)', border: '1px solid rgba(0,255,136,0.3)', color: '#00ff88', padding: '3px 10px', borderRadius: '100px', fontSize: '10px', fontFamily: 'Space Mono,monospace' }}>✓ مكتمل</span>}
                          {inProgress && <span style={{ background: 'rgba(255,215,0,0.1)', border: '1px solid rgba(255,215,0,0.25)', color: '#ffd700', padding: '3px 10px', borderRadius: '100px', fontSize: '10px', fontFamily: 'Space Mono,monospace' }}>⟳ جاري</span>}
                        </div>
                      </div>
                    ...
                    <div style={{ padding: '16px 20px', background: 'rgba(5,10,15,0.5)' }}>
                      ...
                      <div className="prog-bar" style={{ height: '5px', marginBottom: '16px' }}>
                        <div className="prog-fill" style={{ background: isComplete ? 'linear-gradient(90deg, #00ff88, #00d4aa)' : `linear-gradient(90deg, ${course.color}, ${course.color}88)`, width: `${percent}%`, boxShadow: percent > 0 ? `0 0 8px ${course.color}88` : 'none' }}></div>
                      </div>
                      <button style={{ width: '100%', padding: '11px', border: 'none', borderRadius: '10px', background: isComplete ? `linear-gradient(135deg, ${course.color}25, ${course.color}15)` : isHovered ? `linear-gradient(135deg, ${course.color}30, ${course.color}15)` : `${course.color}12`, color: course.color, fontFamily: 'Cairo,sans-serif', fontSize: '14px', fontWeight: '700', cursor: 'pointer', transition: 'all 0.3s', boxShadow: isHovered ? `0 4px 16px ${course.color}25` : 'none', outline: `1px solid ${course.color}30` }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
              ...
              <div style={{ background: 'rgba(10,21,32,0.8)', border: '1px solid #1a3a50', borderRadius: '18px', padding: '24px', backdropFilter: 'blur(10px)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                  ...
                  <span style={{ color: '#1a3a50', fontFamily: 'Space Mono,monospace', fontSize: '11px' }}>{userBadges.length}/{allBadges.length} UNLOCKED</span>
                </div>
                {badgesDisplay.length === 0 ? (
                  <p style={{ color: '#7090a8', textAlign: 'center', fontFamily: 'Cairo,sans-serif', padding: '20px' }}>جاري تحميل الإنجازات...</p>
                ) : (
                  <div className="badges-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '12px' }}>
                    {badgesDisplay.map((badge: any, i: number) => (
                      <div key={badge.id || i} className={`badge-card fade-up ${badge.unlocked ? 'unlocked' : ''}`}
                        style={{ animationDelay: `${i * 0.06}s`, background: badge.unlocked ? `linear-gradient(135deg, ${badge.color}15, ${badge.color}08)` : 'rgba(8,15,24,0.8)', border: `1px solid ${badge.unlocked ? badge.color + '35' : '#1a3a50'}`, opacity: badge.unlocked ? 1 : 0.3 }}>
                        {badge.unlocked && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '2px', background: `linear-gradient(90deg, transparent, ${badge.color}80, transparent)`, borderRadius: '14px 14px 0 0' }}></div>}
                        ...
                        <p style={{ color: badge.unlocked ? 'white' : '#7090a8', fontWeight: '700', fontSize: '12px', marginBottom: '4px' }}>{badge.name}</p>
                        <p style={{ color: '#7090a8', fontSize: '10px', lineHeight: '1.4' }}>{badge.description}</p>
                        {badge.unlocked && <p style={{ color: badge.color, fontSize: '10px', fontFamily: 'Space Mono,monospace', marginTop: '6px', opacity: 0.8 }}>✓ مفتوح</p>}
                      </div>
                    ))}
                  </div>
                )}
